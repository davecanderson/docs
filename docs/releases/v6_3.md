...
Some [OAuth Providers](https://docs.servicestack.net/auth#auth-providers) like Microsoft Graph will have their own roles managed outside of the App. We've added the ability to combine both Microsoft Graph's Azure AD Roles with any App-defined roles which can be managed through the built-in [Admin Users UI](/admin-ui-users) or ServiceStack's [Auth Repository](/auth-repository) or [Assign Roles APIs](/auth-repository#assigning-roles-and-permissions).

This support is only available when using the `OrmLiteAuthRepository` configured to manage roles in distinct tables, required in order to capture the source of each role.
