---
title: v4.0.60 Release Notes
slug: v4-0-60
---
> Quick fix release build that resolves issues reported with Add ServiceStack Reference in v4.0.58.
The v4.0.58 Release Notes below contains new features added in this release:
...
[Typed Users Session](/sessions#using-typed-sessions-in-servicestack)
...
Like all of [ServiceStack's Auth Providers](/authentication-and-authorization)
...
send them using ServiceStack's [.NET Service Clients](/csharp-client):
...
[App Settings](/appsettings) following the
...
[Multitenancy](/multitenancy), the minimal configuration
...
As [mentioned in the last release](/releases/v4_0_56#deprecating-legacy-ormlite-apis)
...
[Custom POCO with all the columns](/ormlite/typed-joins) 
...
[untyped .NET Collections](/ormlite/dynamic-result-sets), we've
...
added to [C#/.NET ServerEventsClient](/csharp-server-events-client)
...
Equivalent Async APIs have been added matching all Sync [HTTP Utils](/http-utils) APIs which are now all mockable, e.g:
