...
[Tailwind CLI](https://tailwindcss.com/docs/installation) enables a responsive, utility-first CSS framework for creating maintainable CSS at scale without the need for any CSS preprocessors like Sass, which is configured to run from an npx script to avoid needing any node_module dependencies.
...
<div data-id="yt-YIa0w6whe2U" class="pb-8">
    <p class="text-center text-lg">For a quick preview of the fast dev workflow of using these components in a Razor Pages App, checkout:</p>
    <div class="flex justify-center">
...
**@servicestack/vue** is our cornerstone library for enabling a highly productive Vue.js development model across our [Vue Tailwind Project templates](/templates-vue) which we'll be continuing to invest in future to build a richer component library that unlocks even greater productivity.
...
<div data-id="AutoCreateForm" class="not-prose"><AutoCreateForm type="CreateBooking" formStyle="card" /></div>
...
Effectively it looks to allow any serializable models and DTOs it can find by looking at all the available heuristics.
